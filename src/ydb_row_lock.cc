...
static void db_txn_note_row_lock(DB *db, DB_TXN *txn, const DBT *left_key, const DBT *right_key) {
    ...
    lt->note_mem_used(new_num_bytes - old_num_bytes);

    toku_mutex_unlock(&db_txn_struct_i(txn)->txn_mutex);
}

void toku_db_txn_escalate_callback(TXNID txnid, toku::locktree *lt, const toku::range_buffer &buffer, void *extra) {
    ...
    if (ttxn != nullptr) {
        ...
        if (r == 0) {
            ...
            lt->note_mem_released(ranges.buffer->get_num_bytes());
            ...
            lt->note_mem_used(ranges.buffer->get_num_bytes());
        } else {
...
int toku_db_start_range_lock(DB *db, DB_TXN *txn, const DBT *left_key, const DBT *right_key,
    ...
    request->set(db->i->lt, txn_anc_id, left_key, right_key, lock_type, txn_anc, toku_is_big_txn(txn_anc));
...
void toku_db_grab_write_lock (DB *db, DBT *key, TOKUTXN tokutxn) {
    ...
    toku::lock_request request;
    request.create();
    request.set(db->i->lt, txn_anc_id, key, key, toku::lock_request::type::WRITE, txn_anc, toku_is_big_txn(txn_anc));
    int r = request.start();
    invariant_zero(r);
